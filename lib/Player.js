...
class Player {
  ...
  draw (context, level, arrayOfBullets) {
  ...
  wormCollision (centipedeSegments,pauseState) {
    centipedeSegments.segmentsArray.forEach((worm, wormIndex) => {
      worm.forEach(indivSeg => {
        if (indivSeg.x > this.x && indivSeg.x < this.x + 20 && indivSeg.y > this.y - 10 && indivSeg.y < this.y + 20){
          ...
          if (this.lives <= 0) {
            ...
            console.log('trying to say game over');
          } else {
            pauseState.paused = 'you-died';
            console.log('trying to say you died');
          }
          // centipedeSegments.segmentsArray.splice(wormIndex,1);
        }
      });
    });
  ...
  spiderCollision (spiderArray,pauseState) {
    console.log(this.y-spiderArray[0].y);
    console.log('-------------');
    if (
