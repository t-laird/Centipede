...
class Gun {
  ...
  spiderCollide(spiderArray, gameLevel) {
    this.arrayOfBullets.forEach((bullet,bulletIndex) => {
      if (bullet.x <= (spiderArray[0].x + 45) && bullet.x >= spiderArray[0].x && bullet.y < (spiderArray[0].y + 30) && bullet.y > spiderArray[0].y){
        this.arrayOfBullets.splice(bulletIndex,1);
        spiderArray.push(new Spider(gameLevel));
        spiderArray[1].isDead = true;
        spiderArray[1].x = spiderArray[0].x;
        spiderArray[1].y = spiderArray[0].y;
        spiderArray[0].x = -500;
        spiderArray[0].y = -500;
        spiderArray[0].dX = 0;
        spiderArray[0].dY = 0;
