  ...
  gameLoop() {
	  if (this.paused === 'start'){
	    ...
	    requestAnimationFrame(this.gameLoop.bind(this));
		  ...
		  this.arrayOfMushrooms.forEach(function (mushroom) {
	    ...
	    if (this.arrayOfSpiders.length !== 0) {
	      this.arrayOfSpiders[0].draw(context);
	      this.arrayOfSpiders[0].move();
	    ...
	    if (this.level >= 1 && Math.floor(Math.random() * 3000) === 0) {
	    	this.arrayOfFleas.push(new Flea(this.level));
	    }

	    this.arrayOfFleas.forEach(function(flea,fleaIndex) {
	      flea.update(this.context, this.arrayOfMushrooms);
	      if (flea.y > 720){
