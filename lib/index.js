...
function gameLoop () {
  if (game.paused === 'start'){
    ...
    // centipede.segmentsArray.forEach((currentSeg, index1) => {
    //   centipede.segmentsArray.forEach((currentSeg2, index2) => {
    //     if (currentSeg === currentSeg2 && index1 !== index2){
    //       centipede.segmentsArray.splice(index, 1);
    //     }
    //   });
    // });

    // iterate through segmentsArraycheck if segments have same x and y as segments in different segmentsArrayif yes and not same index delete
